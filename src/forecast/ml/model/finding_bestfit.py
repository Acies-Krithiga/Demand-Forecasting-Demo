...
class MLBestFitAnalyzer:
    """
    Class to analyze ML forecast results and determine the best fit algorithm
    ...
    def __init__(self, data_df: pd.DataFrame, level_columns=None, actual_column=None):
        """
        Initialize the MLBestFitAnalyzer with data dataframe.
        
        Args:
            data_df (pd.DataFrame): DataFrame containing actual values and ML forecast values
        ...
        # Get ML algorithm columns from config
        self.algorithm_columns = ML_MODELS_CONFIG["available_algorithms"]
        
        # Verify that all algorithm columns exist in the data
        missing_columns = [col for col in self.algorithm_columns if col not in self.data_df.columns]
        if missing_columns:
            logger.warning(f"Missing algorithm columns in data: {missing_columns}")
            self.algorithm_columns = [col for col in self.algorithm_columns if col in self.data_df.columns]
    ...
    def find_best_fit_for_intersection(self, intersection_values: dict) -> Tuple[str, float]:
        """
        Find the best fit ML algorithm for a specific intersection.
        ...
        # Calculate MAPE for each ML algorithm
    ...
    def analyze_all_intersections(self) -> pd.DataFrame:
        """
        Analyze all intersections and return results with intersection and best model columns.
        
        Returns:
            pd.DataFrame: DataFrame with level columns, best_fit_algorithm, best_mape
        """
        logger.info("ML BestFit analysis running...")
        results = []
        ...
        logger.info("ML BestFit analysis finished")
