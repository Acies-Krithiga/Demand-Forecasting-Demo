...
def page_eda():
    ...
    def _write_eda_outputs(sales_df: pd.DataFrame) -> None:
        results = run_eda(sales_df.copy())
        eda_output_dir.mkdir(parents=True, exist_ok=True)
        results["df_sales"].to_csv(eda_output_dir / "df_sales.csv", index=False)
        with open(eda_output_dir / "overall_metrics.json", "w", encoding="utf-8") as f:
            json.dump(results["overall_metrics"], f, ensure_ascii=False, indent=2, default=_json_default)
        results["store_breakdown_metrics"].to_csv(eda_output_dir / "store_breakdown_metrics.csv", index=False)
        results["category_daily_sales"].to_csv(eda_output_dir / "category_daily_sales.csv", index=False)
        results["category_sales_treemap"].to_csv(eda_output_dir / "category_sales_treemap.csv", index=False)
        with open(eda_output_dir / "sku_count_by_cat.json", "w", encoding="utf-8") as f:
            json.dump(results["sku_count_by_cat"], f, ensure_ascii=False, indent=2, default=_json_default)

    def _bootstrap_eda_outputs() -> tuple[bool, str]:
        """Ensure EDA files exist by rebuilding them from local or Drive sales data."""
        missing = [name for name, path in required_files.items() if not path.exists()]
        if not missing:
            return True, "EDA outputs already available."
        ...
        try:
            ...
            missing = [name for name, path in required_files.items() if not path.exists()]
            if not missing:
                return True, "EDA outputs downloaded from Drive and regenerated."
            return False, f"Drive download succeeded but missing outputs remain: {', '.join(missing)}"
        except Exception as e:
            return False, f"Unable to download sales_fact.csv from Drive: {e}"

    ok, bootstrap_message = _bootstrap_eda_outputs()
    if not ok:
        st.error("⚠️ EDA outputs not available!")
        st.info(f"""
        EDA outputs are missing. The app tried to rebuild them automatically but failed.

        1. Go to the Data Upload page
        2. Make sure the Drive file is accessible
        3. Click the Run main.py button

        Detail: {bootstrap_message}

        Missing files: {', '.join([name for name, path in required_files.items() if not path.exists()])}
        """)
        st.stop()
    ...
    try:
        ...
        with open(required_files["overall_metrics"], "r", encoding="utf-8") as f:
            overall_metrics = json.load(f)

        with open(required_files["sku_count_by_cat"], "r", encoding="utf-8") as f:
    ...
    bar_data = store_breakdown_metrics[store_breakdown_metrics["store_id"] != "Overall"].sort_values(
    ...
    selected_store = st.selectbox("Select Store for Detailed Metrics", options=all_stores_options, index=0)
    ...
    if available_cats_in_store:
        ...
        if selected_categories:
            ...
            metric_cols = st.columns(4)
            with metric_cols[0]:
                display_metric_card("No. of SKUs", category_skus)
            with metric_cols[1]:
            ...
            fig_cat_trend = px.line(
                ...
                labels={
                    "date_for_chart": "Date (Weekly Aggregate)",
                    "units_sold": "Total Units Sold (Weekly)",
                    "cat_id": "Category",
                },
                markers=True,
            )
